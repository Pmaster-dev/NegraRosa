...
`;
  ...
  apiRouter.post("/webhooks/test-process", async (req, res) => {
    try {
      ...
      // SECURITY: Avoid hardcoded secrets for HMAC generation to prevent token forgery/tampering risks.
      const hmacSecret = process.env.IDSEC_SECRET_SALT || process.env.JWT_SECRET;
      if (!hmacSecret) {
        throw new Error("IDSEC_SECRET_SALT or JWT_SECRET must be set for secure HMAC generation");
      }
